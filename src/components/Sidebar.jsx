...
const Sidebar = () => {
  ...
  useEffect(() => {
    if (modules.length > 0 && modules[0].chapters.length > 0 && modules[0].chapters[0].videos.length > 0) {
    ...
    <div className="flex flex-col-reverse lg:flex-row pt-16 bg-black items-center justify-center w-full h-screen">
      ...
      <aside
        ...
        className={`min-w-72 w-full overflow-y-scroll relative lg:h-full transition-transform duration-300 ${
          isSidebarOpen ? "lg:col-span-2 lg:w-1/4" : "lg:w-0 lg:hidden lg:-translate-x-full"
        } lg:translate-x-0`}
        aria-label="Sidebar"
      >
        <div className="overflow-y-scroll lg:px-5 lg:h-[100vh] min-h-[50vh] hide-scroll pt-10 px-3 py-3 bg-gray-50 dark:bg-black">
         ...
         { loading? <div className="w-full  text-white justify-center items-center flex h-96">
            <Spinner/>
         </div>:
                  ...
                  {modules.map((module, index) => (
                    <li key={index} className="border-b pb-2 border-b-gray-700">
                      <button
                        ...
                        ${selectedModule.moduleName === module.moduleName ? " text-white" : ""} 
                        ${selectedModule.moduleName === module.moduleName ? "" : ""} dark:text-[#EF4444]`}
                      ...
                      >
                        <div className="flex items-center w-full justify-between">
                          ...
                          <FontAwesomeIcon icon={expandedModules[module.moduleName] ? faChevronUp : faChevronDown} />
                        </div>
                      </button>
                        ...
                        <ul className="space-y-2 pl-4">
                          {module.chapters.map((chapter) => (
                            <li key={chapter._id} className="pt-2">
                              ...
                              >
                                <div className="flex items-center w-full justify-between">
                                  <p className="font-semibold text-gray-800  dark:text-gray-200">{chapter.chapterName}</p>
                                  <FontAwesomeIcon icon={expandedVideos[chapter._id] ? faChevronUp : faChevronDown} />
                                </div>
                              ...
                              {expandedVideos[chapter._id] && ( // Use chapter._id here
                                <ul className="pl-4 space-y-1">
                                  {chapter.videos.map((video) => (
                                    <li key={video._id} onClick={() => handleVideoSelect(video.videoUrl, module)}>
