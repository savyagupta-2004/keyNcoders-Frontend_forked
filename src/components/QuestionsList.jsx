...
const QuestionsList = ({ stepNo, title, questions, theme, moduleIndex, stepIndex }) => {
    ...
    const saveQuestionsState = (updatedQuestions) => {
      localStorage.setItem(`questionsState_${moduleIndex}_${stepIndex}`, JSON.stringify(updatedQuestions));
    };
      ...
      <div className="mb-4 bg-transparent text-black px-4 md:px-8">
        <div
          className={`flex justify-between shadow-md p-4 md:p-6 bg-[#E6E6E6] dark:bg-[#232222] ${
            isVisible ? "bg-gradient-to-r from-[#ED374D] via-[#FA793F] to-[#FCB900] text-black" : "bg-[#232222]"
        ...
        {isVisible && (
          <div className="p-4 md:p-6 bg-gradient-to-r from-[#ED374D] via-[#FA793F] to-[#FCB900] rounded-b-lg">
                    ...
                    <tr key={index}>
                      ...
                      <td className="py-2 px-4 text-center border border-gray-300">
                        <Link to={`/codeEditor/${question.name}`} target="_blank" rel="noopener noreferrer">
                          <img src={logo} alt="Practice Site Logo" className="h-6 inline" />
                        </Link>
                      </td>
                      <td className="py-2 px-4 text-center border border-gray-300">
                        <a href={question.link} target="_blank" rel="noopener noreferrer">
                          <img src={question.logo} alt="Practice Site Logo" className="h-6 inline" />
                        </a>
                      </td>
                      <td className="py-2 px-4 border border-gray-300" style={{ color: getDifficultyColor(question.difficulty) }}>
                        {question.difficulty}
                      </td>
                      <td className="py-2 px-4 border border-gray-300 text-center">
                        <FontAwesomeIcon
                          icon={faPenToSquare}
                          className={`cursor-pointer ${question.flagged ? "text-green-500" : "text-gray-500"}`}
